...
public class SwapFyterzMain extends ApplicationAdapter implements InputProcessor, GestureListener, java.sql.Driver
{
	...
	static final byte ANDROID = 0, DESKTOP = 1, IOS = 2;
	public enum State {
		RUNNING,
		PAUSED,
		STOPPED,
	...
	public void arcadeinit()
	{
		...
		if (isloading)
		{
			...
			if (loader.update())
			{
				...
				// scenes.
				land = new SPFZSceneLoader(rm, SwapFyterzMain.this, "", "");
				...
				if (INTRO == 2 && Gdx.input.isTouched() || gamestart)
				{
					...
					if (uicomplete == true)
					{
						if (!dialog)
						{
							...
							root.getChild("meconst").getEntity().getComponent(ButtonComponent.class)
								.addListener(new ButtonComponent.ButtonListener()
								{
							...
							root.getChild("treyconstbtn").getEntity().getComponent(ButtonComponent.class)
								.addListener(new ButtonComponent.ButtonListener()
								{
							...
							root.getChild("miguelconstbtn").getEntity().getComponent(ButtonComponent.class)
								.addListener(new ButtonComponent.ButtonListener()
								{
							...
							root.getChild("mikloconstbtn").getEntity().getComponent(ButtonComponent.class)
								.addListener(new ButtonComponent.ButtonListener()
								{
							...
							root.getChild("creditbtn").getEntity().getComponent(ButtonComponent.class)
								.addListener(new ButtonComponent.ButtonListener()
								{
